...
public class TagConventions : HtmlConventionRegistry
{
    public TagConventions()
    {
        ...
        Editors
            ...
            .ModifyWith(m =>
                m.CurrentTag.AddPattern("9{1,2}/9{1,2}/9999")
                    .AddPlaceholder("MM/DD/YYYY")
                    .AddClass("datepicker")
                    .Value(
                        m.Value<DateTime?>() != null
                            ? m.Value<DateTime>().ToShortDateString()
                            : string.Empty
                    )
            );
        Editors
        ...
        Editors
            .IfPropertyIs<byte[]>()
            .ModifyWith(m => m.CurrentTag.Value(Convert.ToBase64String(m.Value<byte[]>())));
        ...
        Labels.Always.ModifyWith(er =>
            er.CurrentTag.Text(er.CurrentTag.Text().Replace("Data ", ""))
        );
